...
export async function POST(request: Request) {
  try {
    ...
    if (query.includes('preço') || query.includes('quanto') || query.includes('valor') || query.includes('corte') || query.includes('barba') || query.includes('combo')) {
      ...
      reply =
        'Estamos atendendo de Segunda a Sábado, das 09:00 às 19:00! Barbeiro Alemão na ativa. Qual horário fica melhor para você dar aquele tapa no visual?';
      ...
      reply =
        `Fala parceiro! Aqui é o assistente virtual da Barbearia do Alemão 777. Posso te ajudar a agendar um horário com o Alemão, conferir a tabela de preços ou tirar qualquer dúvida. Como posso te ajudar hoje?`;
    }
